#!/usr/bin/env python3
"""
Identificação de Parâmetros do Modelo SIR usando Dados COVID-19 do Brasil

Este script orquestra o workflow completo de identificação:
1. Carregar dados COVID-19 do Brasil
2. Preparar compartimentos SIR
3. Estimar parâmetros β e γ usando mínimos quadrados
...
from src.utils.data_loader import load_covid_data_brazil, prepare_sir_data
from src.identification.least_squares import identify_parameters
...
def main():
    ...
    Orquestra todo o workflow de identificação de parâmetros SIR
    """
    try:
        ...
        logger.info("=" * 70)
        logger.info("IDENTIFICAÇÃO DE PARÂMETROS SIR - COVID-19 BRASIL")
        logger.info("=" * 70)
        ...
        # 3. Preparar compartimentos SIR
        logger.info("\n🔬 Preparando compartimentos SIR...")
        sir_data = prepare_sir_data(
            df, N,
            smooth=config['data']['use_smoothing'],
            window=config['data']['smooth_window']
        )

        logger.info(f"✓ Período: {sir_data['dates'][0].date()} a {sir_data['dates'][-1].date()}")
        logger.info(f"✓ Número de dias: {len(sir_data['time'])}")
        logger.info(f"\n📊 Condições iniciais:")
        logger.info(f"   S₀ = {sir_data['S'][0]:>15,.0f} suscetíveis")
        logger.info(f"   I₀ = {sir_data['I'][0]:>15,.0f} infectados")
        logger.info(f"   R₀ = {sir_data['R'][0]:>15,.0f} removidos")
        logger.info(f"   N  = {sir_data['population']:>15,.0f} população total")
        ...
        results = identify_parameters(
            time_data=sir_data['time'],
            I_data=sir_data['I'],
            R_data=sir_data['R'],
            N=N,
        ...
        if config['output']['save_parameters']:
            ...
            params_file = params_dir / 'sir_params.json'
            ...
            results_json = {
                ...
                'config': config,
                'data_period': {
                    'start': str(sir_data['dates'][0].date()),
                    'end': str(sir_data['dates'][-1].date()),
                    'days': len(sir_data['time'])
                },
                'initial_conditions': {
                    'S0': float(sir_data['S'][0]),
                    'I0': float(sir_data['I'][0]),
                    'R0': float(sir_data['R'][0])
                }
            }
