#!/usr/bin/env python3
"""
Optimal Control — SEIR Model (Realistic Economic Cost)
=======================================================
Applies the Pontryagin Maximum Principle to the SEIR model using a
...
Usage:
    python scripts/optimal_control_pontryagin_seir2.py
"""
...
from src.control.cost_functions import RealisticCostSEIR
...
MODEL_NAME = 'SEIR2'
FIGURES_DIR = ROOT_DIR / 'results' / 'figures' / 'control' / 'pontryagin_seir2'
RESULTS_DIR = ROOT_DIR / 'results' / 'control' / 'pontryagin_seir2'
...
COSTATE_LABELS = {
    ...
    'p_E': 'Co-state p_E(t)  [R$/person]',
    'p_I': 'Co-state p_I(t)  [R$/person]',
    'p_R': 'Co-state p_R(t)  [R$/person]',
}
...
def main():
    ...
    print("=" * 70)
    print("SEIR OPTIMAL CONTROL — REALISTIC ECONOMIC COST")
    print("Pontryagin Maximum Principle | Shooting Method")
    print("=" * 70)

    # --- Model parameters ---
    beta  = seir_cfg['model']['beta']
    sigma = seir_cfg['model']['sigma']
    gamma = seir_cfg['model']['gamma']
    N     = seir_cfg['model']['N']

    print(f"\nModel parameters (literature, Brazil 2020):")
    print(f"  beta  = {beta}   (transmission rate)")
    print(f"  sigma = {sigma:.3f}  (incubation rate, 1/5.1 days)")
    print(f"  gamma = {gamma}   (recovery rate, 10-day period)")
    print(f"  R0    = {beta/gamma:.2f}")
    print(f"  N     = {N:,.0f}")

    # --- Initial conditions ---
    ic = seir_cfg['initial_conditions']
    x0 = np.array([ic['S0'], ic['E0'], ic['I0'], ic['R0_ic']])
    print(f"\nInitial conditions:")
    for k, v in zip(['S', 'E', 'I', 'R'], x0):
        print(f"  {k}0 = {v:>14,.0f}")

    # --- Cost function ---
    cost = RealisticCostSEIR(
    ...
    save_json(RESULTS_DIR / 'trajectories.json', {
        ...
        'model': 'SEIR', 'cost_type': 'realistic_economic',
